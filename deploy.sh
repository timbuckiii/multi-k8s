...
kubectl apply -f k8s
kubectl set image deployments/server-deployment server=timbuckiii/multi-server:$SHA
kubectl set image deployments/client-deployment client=timbuckiii/multi-client:$SHA
kubectl set image deployments/worker-deployment worker=timbuckiii/multi-worker:$SHA